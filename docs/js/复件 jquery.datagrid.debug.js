...
;(function($){
	...
	$.dataGrid.extend({
		...
		tpl : baidu.template,
		_Tpl : {
			'_view1' : '<div class="datagrid-view1" style="width:<%=rowNumbersWidth%>;height:100%;">'
									...
									+'<div class="datagrid-header-inner-wraper">'
										+'<table class="datagrid-htable" border="0" cellspacing="0" cellpadding="0">'
											+'<tbody>'
												...
												+'<td class="datagrid-td-rownumber"><div class="datagrid-header-rownumber" style="width:<%=(rowNumbersWidth)%>px;"></div></td>'
												+'</tr>'
											+'</tbody>'
										+'</table>'
									+'</div>'
								+'</div>'
								+'<div class="datagrid-header-outer">'
									+'<div class="datagrid-header-outer-wraper">'
										+'<table class="datagrid-locktable" border="0" cellspacing="0" cellpadding="0">'
											+'<tbody>'
											+'</tbody>'
										+'</table>'
									+'</div>'
									...
									+'<table class="datagrid-btable" cellspacing="0" cellpadding="0" border="0">'
										+'<tbody>'
											...
											+'<tr id="<%=id%>-view1-row-<%=i%>" style="<%=(groupBy !== false) ? "display:none" : ""%>" <%=( isCreate && typeof data[i]["_groupid_"] != "undefined" ) ? "datagrid-group-id="+data[i]["_groupid_"] : ""%> datagrid-row-index="<%=i%>" datagrid-row-select="0" class="datagrid-row datagrid-row-view1">'
												+'<td align="center" class="datagrid-td-rownumber"><div class="datagrid-cell-rownumber" style="width:<%=parseInt(rowNumbersWidth)%>px;"><%var j = i%><% var _exp = ( rowNumbersExpand === false ? j++ : baidu.template(rowNumbersExpand,data[i]) )%><%=_exp%></div></td>'
											+'</tr>'
											+'<% } %>'
										+'</tbody>'
									+'</table>'
	...
	$.dataGrid.fn.extend({
		...
		onViewSizeChange : function(){
			...
			$(gid).find(".datagrid-header-inner").height( view2_header_h );
			...
			$(gid).find(".datagrid-header-outer").height( view2_header_outer_h );
			//设置datagrid-header
			var header_h = parseInt( view2.find(".datagrid-header .datagrid-header-outer").outerHeight() ) + parseInt( view2.find(".datagrid-header .datagrid-header-inner").outerHeight() );
			
			view1.find(".datagrid-header").height( header_h );
			view2.find(".datagrid-header").height( header_h );
		...
		setView : function(){
			var self = this,
				...
				//debug start
				var render = template.compile(self.getTpl("_view1"));
				console.log(render);
				console.log(render(opt));
				//debug end
