...
;(function($){
			...
			'view2_header_inner_row' : '<tr class="datagrid-header-row">'
											...
											+'<td field="<%=fields[i]["field"]%>" align="<%=fields[i]["align"]%>">'
												+'<div class="datagrid-header-wrap" field="<%=fields[i]["field"]%>" >'
													+'<div class="datagrid-cell datagrid-cell-header-<%=fields[i]["field"]%> <%=fields[i]["hcls"]%>"  style="width:<%=fields[i]["width"]%>">'
	...
	$.dataGrid.fn.extend({
		...
		setFieldValue : function(rid,field,value){
			...
			self.fireEvent('onCellEdit',[t,rid,field,rowData]);
			return rowData;
		},
		...
		setFieldWidth : function(field,width){
			...
			$("#"+opt.id+"_css_"+field).remove();
			var style = [];
			style.push('<style type="text/css" id="');
			style.push(opt.id);
			style.push('_css_');
			style.push(field);
			style.push('">#');
			style.push(opt.id);
			style.push(' .datagrid-cell-header-');
			style.push(field);
			style.push(',#');
			style.push(opt.id);
			style.push(' .datagrid-cell-c1-');
			style.push(field);
			style.push('{width:');
			style.push(width);
			style.push('px !important;}</style>');
			style = style.join("");
			$("#"+opt.id).before(style);
		...
		onHeaderCreate : function(){
			...
			headerBody.find(">tr td[field],>tr th[field],>tr td["+_thex.options_from+"],>tr th["+_thex.options_from+"]").each(function(i){
				...
				$($this).html('<div class="datagrid-header-wrap" field="'+_d4.field+'"><div class="datagrid-cell datagrid-cell-header-'+_d4.field+' '+_d4.hcls+'" style="width:'+parseFloat(_d4.width)+'px"><span>'+_d4.title+'</span><span class="datagrid-sort-icon">&nbsp;</span></div></div>');
		...
		bindRowEvent : function( tr,ltr ){
			...
			/*	'mouseleave' : function(e){
				...
				'contextmenu' : function(ev){
					var rowId = $(this).attr("datagrid-row-index");
					var rowData = self.getRowData(rowId);
					var r = self.fireEvent('onRowContextMenu',[this,rowId,rowData,ev]);
					if(r == false) return false;
				}
			};
