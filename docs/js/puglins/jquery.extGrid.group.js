	...
	$.dataGrid.fn.extend({
		fixIEHack : function(){
			...
			if( isIE7 || isIE6 ) {
				...
				if( !last ) return;
				var _w = last['width'];
				var w = $("#view2-datagrid-header-inner-htable-tbody-"+opt.id).find(">tr.datagrid-header-row div.datagrid-cell-header-"+last.field)._outerWidth();
		...
		addGroupRow : function(isFirst){//isFirst = true 隐藏所有行
			...
			//隐藏
			if(isFirst) {
				$("#view2_"+opt.id).find("tr.datagrid-row").hide();
				$("#view1_"+opt.id).find("tr.datagrid-row").hide();
			}
		...
		setGroupRowSize : function(){
			...
			if(!opt.groupBy || !opt.showGroupRow ) return;	
			var render = gid+" .datagrid-view2";
			$("#view1-datagrid-body-btable-tbody-"+opt.id).find(">tr.datagrid-group-row-view1 >td").attr("colspan",opt.lockColumns.length + 1);
			var w = $("#view2-datagrid-header-inner-htable-tbody-"+opt.id).find(">tr.datagrid-header-row:first")._outerWidth();
			$("#view2-datagrid-body-btable-tbody-"+opt.id)
				.find("tr.datagrid-group-row >td")
