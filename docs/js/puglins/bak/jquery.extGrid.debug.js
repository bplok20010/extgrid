	...
	$.dataGrid.fn.extend({
		...
		*/
		...
		setFieldWidth : function(field,width,real){
			...
			//self.setGroupRowSize();//迁移到group.js
			//self.setExpandRowSize();//迁移到expandrow.js
			/*检查文字是否超出边界*/
		...
		forceFitColumn : function( fields,_w ){//平均设置列宽大小，如果传入field则传入的filed 大小不会改变
			...
			//self.setGroupRowSize();//迁移到group.js
			//self.setExpandRowSize();//迁移到expandrow.js
			self.fireEvent("onForceFitColumn",[]);
			return self;
		},
...
//				}
		...
		setGridBody : function(render,func){
			...
			var br = self.fireEvent('onBeforeShowGrid',[]);
			if( br === false ) {
				return false;	
			}
			...
			setTimeout(function(){
				//对数据分类
				//self.groupByField();//迁移到 group.js
		...
		afterGridShow : function(lazy){
			...
			//self.setFooterRow();//迁移到footer.js
			...
			if( !lazy ) {
				//更新
				//self.addGroupRow(true);//迁移到group.js
