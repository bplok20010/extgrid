	...
	$.fn.togrid = function(cfg,_cfg){//如果url='' 只能处理简单的table比如 表头不会包含的合并之类的
		...
		this.each(function(i){
			...
			thead.each(function(i){
				...
				column.field = column.field ? column.field : "field_"+$.dataGrid.aid++;//(Math.floor(Math.random() * 999999));
			...
			opt.renderTo = $("<div id='"+ ($(this).attr('id') ? $(this).attr('id') : '')+"' class='datagrid "+ $(this).attr('class') +"'></div>");
			
			$(this).after(opt.renderTo).remove();
