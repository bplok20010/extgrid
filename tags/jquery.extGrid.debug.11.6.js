	...
	$.dataGrid.fn.extend({
		...
		addPagerEvent : function(){
			...
			obj.find(".pagination-num").keydown(function(e){
				if(e.keyCode === 13) {
					opt.pageNumber = parseInt( obj.find(".pagination-num").val() );	
					opt.pageNumber = self.isNumber( opt.pageNumber ) ? opt.pageNumber : 1;
					self.refreshData();	
				}
			});
			return self;
		...
		getRowData : function (rid,isPK){
			...
			if(!isPK) {
				//return data[rid];
				return $("#"+opt.id+"-row-"+rid).data('rowData');
			} else {
				var pk = opt.pk;
				var i=0,
					len = data.length;
		...
		setRowData : function (rid,field,value){
			...
			var data = $("#"+opt.id+"-row-"+rid).data('rowData');
		...
		setFieldValue : function(rid,field,value){
			...
			*/
			...
			self.fireEvent('onCellEdit',[t,rid,field,value,rowData]);
			return rowData;
		},
...
//				}
		...
		_insert : function(rid,d,ai){
			...
			if( ai ) {
				...
				rows.each(function(i,t){
					rids.push( Number($(t).attr("datagrid-row-index")) );				   
				});
				rids.push(rid);
		...
		deleteRow : function(rid){
			...
			for( var i=0;i<opt.data;i++ ) {
				if( opt.data[opt.pk] == data[opt.pk] ) {
		...
		parseRowTpl : function(tr,rid,d){
			...
			tr.find(">td,>th").each(function(f){
				...
				var $this = this;
				if( $(this).is("th") ) {
					$this = $("<td id='"+tdId+"' field='"+fields[f]['field']+"' align='"+fields[f]['align']+"'><div  id='"+cellId+"'  class='datagrid-cell datagrid-cell-"+fields[f]['field']+" datagrid-cell-c1-"+fields[f]['field']+" ' >"+$(this).html()+"</div></td>");//style='width:"+fields[f]['width']+";'
					$(this).replaceWith( $this );
				} else {
					$(this).attr("field",fields[f]['field'])
						   .attr("align",fields[f]['align'])
						   .attr("id",tdId)
						   .html("<div  id='"+cellId+"' class='datagrid-cell datagrid-cell-"+fields[f]['field']+" datagrid-cell-c1-"+fields[f]['field']+" ' >"+$(this).html()+"</div>");//style='width:"+fields[f]['width']+";'
				}						 
			});
...
//		},
		...
		setWH : function(width,height){
			var self = this;
			self.fireEvent("onSizeChange",[width,height]);
			return self;
		},
		//设置参数
	...
	$.fn.moveColumn = function(opt) {
		...
		columns.bind("mousemove.h",function(e){
			...
			var r = opt.self.fireEvent("onColumnMoving",[opt]);
			if(r === false) {
				return;	
			}
		...
		function start(e) {
			...
			$(document.body).bind("mouseup.move",function(e){
				...
				opt.self.fireEvent("onColumnMove",[opt]);
				e.preventDefault();
				e.stopPropagation();
			});
		}
