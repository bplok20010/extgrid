...
;(function($){
	...
	$.dataGrid.fn.extend({
		...
		_lockColumn : function(field) {
			...
			var f = $("#view2-datagrid-header-inner-htable-tbody-"+opt.id).find(">tr.datagrid-header-row td[field='"+field+"']");
			
			$(view1_header_hbody_id).find(">tr.datagrid-header-row").append(f);
			
			$("#view2-datagrid-header-outer-locktable-tbody-"+opt.id).find(">tr.datagrid-row td[field='"+field+"']").each(function(i){
			...
			$("#view2-datagrid-body-btable-tbody-"+opt.id).find(">tr.datagrid-row td[field='"+field+"']").each(function(i){
				var rowId = $(this).parent().attr("datagrid-rid");
				var getRn = self._getRowNumber(rowId);
				if( getRn.isNew ) {
		...
		_unLockColumn : function(field) {
			...
			if( pos < 0) {
				...
				f.insertAfter( $("#view2-datagrid-header-inner-htable-tbody-"+opt.id).find(">tr.datagrid-header-row").find("td[field='"+_field+"']"));	
			}
			//移动单元格
