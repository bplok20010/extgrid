...
;(function($){
	...
	$.dataGrid.extend({
		...
		_Tpl : {
			...
			'view1_row' : '<tr id="<%=id%>-view1-row-<%=i%>" <%=( typeof data["_groupid_"] != "undefined" ) ? "datagrid-group-id="+data["_groupid_"] : ""%> datagrid-row-index="<%=i%>" datagrid-row-select="0" class="datagrid-row datagrid-row-view1 <%=((i+1)%2 ? "datagrid-row-view1-single" : "datagrid-row-view1-double")%>">'
						  		+'<td id="<%=id%>_row_<%=i%>_td_rownumber" align="center" class="datagrid-td-rownumber"><div id="<%=id%>_row_<%=i%>_cell_rownumber" class="datagrid-cell-rownumber" style="width:<%=parseFloat(rowNumbersWidth)%>px;"><%var j = i%><%=(rowNumbersExpand === false ? ++j : opt.self.tpl(rowNumbersExpand,data))%></div></td>'
						   +'</tr>',
			'view2_row' : '<tr id="<%=id%>-row-<%=i%>" <%=( typeof data["_groupid_"] != "undefined" )? "datagrid-group-id="+data["_groupid_"] : ""%> datagrid-row-index="<%=i%>" datagrid-row-select="0" class="datagrid-row <%=((i+1)%2 ? "datagrid-row-single" : "datagrid-row-double")%>">'
						   ...
						   +'<% var field = fields[j]["field"];%>'
						   		+'<td field="<%=fields[j]["field"]%>" id="<%=id%>_<%=fields[j]["field"]%>_row_<%=i%>_td" align="<%=fields[j]["align"]%>">'
										+'<div id="<%=id%>_<%=fields[j]["field"]%>_row_<%=i%>_cell" class="datagrid-cell datagrid-cell-c1-<%=fields[j]["field"]%> <%=fields[j]["bcls"]%>" style="width:<%=fields[j]["width"]%>;" ><%=fields[j]["_expand"] !== false ? opt.self.tpl(fields[j]["_expand"],data) : data[field]%></div>'
								+'</td>'
							+'<% } %>'
							+'</tr>'
		}
	...
	$.dataGrid.fn.extend({
		...
		setPager : function(init) {
			...
			if(opt.id)
			var _pager = $("#"+opt.id).find(">.datagrid-pager");
...
//				}
		...
		cellEvents : function(rowId,e) {
			...
			var cell = $(target);
			if( !cell.hasClass("datagrid-cell") ) {
				cell = cell.parents(".datagrid-cell");	
				if( !cell.size() ) {
					cell = $("div.datagrid-cell",$(target));
				}
			}
			...
			switch( e.type ) {
				case 'click' :
					self.fireEvent('onClickCell',self,[cell.eq(0),rowId,field,value,e]);
					break;
				case 'dblclick' :
					self.fireEvent('onDblClickCell',self,[cell.eq(0),rowId,field,value,e]);
					break;
				case 'mouseover' : //case 'mouseenter' : 
					self.fireEvent('onOverCell',self,[cell.eq(0),rowId,field,value,e]);
					break;
				case 'mouseout' :
					self.fireEvent('onOutCell',self,[cell.eq(0),rowId,field,value,e]);
					break;
			}
		},
		//行 生成
		...
		view2_row : function(d){
			...
			for(var j in d.fields) {
				...
				var _expand = d.fields[j]["_expand"] !== false ? opt.self.tpl(d.fields[j]["_expand"],d.data) : d.data[field];
